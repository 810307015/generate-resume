  ...
  return (
    <div className="ScrollBar" style={{ width, height }} ref={container => containerRef = container}>
      ...
      <div className={`scroll-bar-control ${autoHide ? 'autoHide' : ''} ${isScrolling ? 'active' : ''}`}>
        ...
        {
          mode !== 'x' && isShowControlY && <div className="scroll-bar-control-common scroll-bar-control-y" style={{ height: yHeight, transform: `translateY(${yTop}px)` }} onClick={handleYClick}></div>
        }
      </div>
    </div>
  );
