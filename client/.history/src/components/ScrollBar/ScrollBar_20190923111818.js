...
function ScrollBar(props) {
  ...
  const [xMarginLeft, setXMarginLeft] = useState(false); // x轴滚动条距离左侧的偏移量
  const [yMarginTop, setYMarginTop] = useState(false); // y轴滚动条距离顶部的偏移量
  ...
  return (
    <div className="ScrollBar" style={{ width, height }} ref={container => containerRef = container}>
      ...
      <div className={`scroll-bar-control ${autoHide ? 'autoHide' : ''} ${isScrolling ? 'active' : ''}`}>
        {
          mode !== 'y' && isShowControlX && <div className="scroll-bar-control-common scroll-bar-control-x" style={{ width: xWidth, marginLeft: xMarginLeft }} onClick={handleXClick}></div>
        }
        {
          mode !== 'x' && isShowControlY && <div className="scroll-bar-control-common scroll-bar-control-y" style={{ height: yHeight, marginTop: yMarginTop }} onClick={handleYClick}></div>
        }
      </div>
    </div>
  );
