...
function ScrollBar(props) {
  ...
  let xWidth = 0; // x轴滚动条的宽度
  let yHeight = 0; // y轴滚动条的高度

  const caculate = () => {
    const { offsetWidth: conWidth, offsetHeight: conHeight } = containerRef;
    ...
    (cHeight > conHeight) ? setIsShowControlY(true) : setIsShowControlY(false);
  ...
  return (
    <div className="ScrollBar" style={{ width, height }} ref={container => containerRef = container}>
      ...
      <div className={`scroll-bar-control ${autoHide ? 'autoHide' : ''} ${isScrolling ? 'active' : ''}`}>
        {
          mode !== 'y' && isShowControlX && <div className="scroll-bar-control-common scroll-bar-control-x" style={{ width: xWidth }}></div>
        }
        {
          mode !== 'x' && isShowControlY && <div className="scroll-bar-control-common scroll-bar-control-y" style={{ height: yHeight }}></div>
        }
      </div>
    </div>
  );
